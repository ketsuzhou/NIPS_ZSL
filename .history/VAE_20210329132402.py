...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance):
        ...
        # encoder parameteres
        self.num_deter_enc = args.num_deter_enc_blocks  # each halfs the height and width
        ...
        c_scaling = CHANNEL_MULT**(self.num_deter_enc +
                                   self.num_latent_scales - 1)
        spatial_scaling = 2**(self.num_deter_enc +
                              self.num_latent_scales - 1)
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                ...
                spatial_scaling = self.in_shape * 2**(
                    self.num_latent_scales - s) / (2** self.num_deter_enc)
                nf_in_shape = [
