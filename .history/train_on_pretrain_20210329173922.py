...
def arg_parse():
    ...
    parser.add_argument('--cfg_file', type=str, dest='cfg_file', default='configs/self_adaptation/ResNet101_AwA2_SS_C.yaml')
    ...
    parser.add_argument('--dataset', type=str, default='AwA2',
                        choices=[], help='which dataset to use')
    parser.add_argument('--split', default='standard_split', metavar='NAME',
    ...
    parser.add_argument('--use_pre', action='store_true', default=False, help='use pre-extracted feature')

    # Flow params
    parser.add_argument('--num_channels', default=96, type=int, help='Number of channels in Flow++')
    parser.add_argument('--num_ConvAttnBlock', default=1, type=int, help='Number of ConvAttnBlock')
    parser.add_argument('--num_components', default=2, type=int, help='Number of components in the mixture')
    parser.add_argument('--use_self_attn', type=bool, default=True, help='Use attention in the coupling layers')
...
def config_process(config):
    ...
    config.image_dir = os.path.join(config.data_root, config.dataset, 'images/')
    config.class_file = os.path.join(config.data_root, config.dataset,  'classes.txt')
    config.image_label = os.path.join(config.data_root, config.dataset, 'image_labels.txt')
    config.attributes_file = os.path.join(config.data_root, config.dataset, 'class_attributes.txt')

    config.train_classes = os.path.join(config.data_root, config.split, config.dataset, 'trainvalclasses.txt')
    config.test_classes = os.path.join(config.data_root, config.split, config.dataset, 'testclasses.txt')
...
def main(args):
    ...
    cpc_v2 = CPCV2.load_from_checkpoint(
        weight_path, strict=False)
    cpc_v2.freeze()
    # finetuner
    finetuner = SSLFineTuner(cpc_v2, in_features=cpc_v2.z_dim, num_classes=cpc_v2.num_classes)
