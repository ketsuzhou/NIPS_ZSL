...
class EncCombinerCell(nn.Module):
    def __init__(self, Cin_decodered, Cout_decodered, cell_type):
        super(EncCombinerCell, self).__init__()
        self.cell_type = cell_type
        # Cin = Cin1 + Cin2
        self.conv = Conv2D(Cin_decodered, Cout_decodered, kernel_size=1, stride=1, padding=0, bias=True)

    def forward(self, encodered, decodered):
        decodered = self.conv(decodered)
        out = encodered + decodered
        return out
