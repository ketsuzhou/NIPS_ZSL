...
class AutoEncoder(nn.Module):
    ...
    def init_stochastic_encoder(self):
        ...
        for s in range(self.num_latent_scales):
            ...
            if not s == self.num_latent_scales - 1:
                cell = EncCombinerCell(2 * half_in_chan_stoch_enc,
                                        half_in_chan_stoch_enc,
        ...
        for s in range(self.num_latent_scales):
            if not s == 0:
                ...
                cell = Conv2D(half_in_chan_stoch_enc,
                                2 * self.residul_latent_dim + 2 * self.nf_dim_per_scale[s],
                                kernel_size=3,
                                padding=1,
                                bias=True)
    ...
    def reparametrization(mu, log_sig):
        std = 0.5 * torch.exp(log_sig)
    ...
    def forward(self, x):
        ...
        mu_p, log_sig_p = torch.chunk(s, 2, dim=1)
        z_non_local = self.reparametrization(mu_p, log_sig_p)

        z, _ = z_non_local.sample()
        ...
        mu_z0 = torch.zeros(
            self.batch_size,
            self.half_in_chan_stoch_enc - self.nf_dim_per_scale, z.size(2),
            z.size(3))
