...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        z_loss1 = self.global_nf_prior(z_global)
        # decode
        s = self.up1(z_global)
        s = self.dec(s)
        ...
        z_loss2 = self.local_nf_prior_z(local_nf_condition, z_local)

        r = self.reparametrization(mu2, log_var2)
        # self.local_nf_prior_r(local_nf_condition, z_local)

        torch.cat(decoded_feature, z_local, r, dim=1)
