...
class invertible_net(Ff.ReversibleGraphNet):
    def __init__(self,
                 ...
                 use_split,
                 downsample,
                 FlowBlocks_architecture,
                 in_shape,
                 verbose=True,
                 mid_channels=96,
                 num_ConvAttnBlock=10,
                 num_components=32,
                 drop_prob=0.2,
                 num_InvAutoFC=1):
        ...
        for block_index in range(len(FlowBlocks_architecture)):
            ConditionNode = Ff.ConditionNode(
                feature_channels,
                img_dims[0],
                img_dims[1],
            ...
            nodes = Flow_Block(
                ...
                drop_prob=drop_prob)
            nodes, in_shape = reduce_spacial_dimension(
                nodes,
                in_shape,
                block_index,
        ...
        for _ in range(num_InvAutoFC):
            nodes.append(
                Ff.Node(nodes[-1].out0, Fm.ActNorm, {}, name=f'ActNorm_final'))
            nodes.append(
                Ff.Node(nodes[-1].out0, Fm.InvAutoFC, {}, name='InvAutoFC'))
...
def Flow_Block(nodes, ConditionNode, block_index, in_shape,
               FlowBlocks_architecture, mid_channels, num_ConvAttnBlock,
               num_components, drop_prob, use_self_attn):
    ...
    for i in range(num_channelwise):
        ...
        nodes.append(
            Ff.Node([nodes[-1].out0],
                    Fm.PermuteRandom, {'seed': np.random.randint(2**31)},
...
def reduce_spacial_dimension(nodes,
                             in_shape,
                             block_index,
                             use_split,
                             downsample=None):
