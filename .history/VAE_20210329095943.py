...
class AutoEncoder(nn.Module):
    def __init__(self, args, writer, arch_instance):
        ...
        c_scaling = CHANNEL_MULT**(self.num_deter_enc_blocks +
                                   self.num_latent_scales - 1)
        spatial_scaling = 2**(self.num_deter_enc_blocks +
                              self.num_latent_scales - 1)
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[s]):
                ...
                cell = Cell(self.in_chan_stoch_enc
                            if g == 0 else self.half_in_chan_stoch_enc,
                ...
                if not (s == self.num_latent_scales - 1
                    ...
                    cell = EncCombinerCell(self.half_in_chan_stoch_enc,
                                           self.half_in_chan_stoch_enc,
            ...
            if s < self.num_latent_scales - 1:
                ...
                cell = Cell(self.half_in_chan_stoch_enc,
                            self.in_chan_stoch_enc,
                            cell_type='down_enc',
                            arch=arch,
                            use_se=self.use_se)
                enc_tower.append(cell)
    ...
    def init_pri_pos_sampler(self, args):
        posterior_sampler, norm_prior_sampler, condition_encoder = nn.ModuleList(
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                ...
                cell = Conv2D(self.half_in_chan_stoch_enc,
                ...
                spatial_scaling = self.in_shape * 2**(
                    self.num_latent_scales - s) / (2** self.num_deter_enc_blocks)
                nf_in_shape = [
                    self.nf_dim_per_group, spatial_scaling, spatial_scaling
                ]
            ...
            for g in range(self.groups_per_scale[self.num_latent_scales - s -
                ...
                if not (s == 0 and g == 0):
                    ...
                    cell = Cell(
                        ...
                        self.groups_per_scale[self.num_latent_scales - s - 1] -
                        1 else self.half_in_chan_stoch_enc,
    ...
    def decoder_output(self, logits):
        if self.dataset == 'mnist':
            return Bernoulli(logits=logits)
                ...
                'cifar10', 'celeba_64', 'celeba_256', 'imagenet_32',
