...
class BaseTrainer(object):
    ...
    def __init__(self, model, run_on_gpu=True, multi_gpu=True, double_precision=False):
        ...
        if args.backbone == 'resnet101':
            self.backbone = models.__dict__['resnet101'](pretrained=True).to(self.device, self.dtype)
        else:
            weight_path = 'https://pl-bolts-weights.s3.us-east-2.amazonaws.com/cpc/cpcv2_weights/checkpoints/epoch%3D526.ckpt'
            self.backbone = CPCV2.load_from_checkpoint(weight_path, strict=False).to(self.device, self.dtype)
        ...
        logger.info(
            ...
            )) if self.multi_gpu else "GPU" if self.run_on_gpu else "CPU",
    ...
    def wrap_up_early_stopping(self, best_model, currrent_loss, best_loss, best_epoch):
        ...
        if loss_ is None or best_loss is None:
            ...
            logger.info("Early stopping after epoch %s, with loss %8.5f compared to final loss %8.5f",
                        best_epoch + 1, best_loss, loss_)
    ...
    def report_epoch(i_epoch, loss_labels, loss_train, loss_val, loss_contributions_train, loss_contributions_val, verbose=False):
        ...
        try:
            train_report = "Epoch {:>3d}: train loss {:>8.5f} +/- {:>8.5f} ({})".format(
                i_epoch + 1, loss_train[0], loss_train[1], contribution_summary(
                    loss_labels, loss_contributions_train)
            )
        except:
    ...
    def _check_for_nans(label, *tensors, fix_until=None, replace=0.0):
        for tensor in tensors:
            ...
            if torch.isnan(tensor).any():
                ...
                logger.warning(
                    "%s contains %s NaNs, aborting training!", label, n_nans)
                raise NanException
...
class Trainer(BaseTrainer):
    ...
    def train(
        ...
        if early_stopping and early_stopping_patience is None:
            ...
            logger.debug("Using early stopping with patience %s",
        ...
        for i_epoch in range(initial_epoch, epochs):
            ...
            self.report_epoch(i_epoch, loss_labels, loss_train, loss_val,
                              loss_contributions_train, loss_contributions_val, verbose=verbose_epoch)

            # Callbacks
            if callbacks is not None:
                for callback in callbacks:
                    callback(i_epoch, self.model, loss_train,
                             loss_val, last_batch=self.last_batch)
    ...
    def epoch(
        ...
        if val_loader is not None:
            ...
            for i_batch, batch_data in enumerate(val_loader):
                batch_loss, batch_loss_contributions = \
                    self.batch_val(batch_data, loss_functions, loss_weights,
                                   forward_kwargs=forward_kwargs,
                                   custom_kwargs=custom_kwargs)
                ...
                self.report_batch(i_epoch, i_batch, False,
                                  batch_data, batch_loss)
    ...
    def partial_epoch(
        ...
        n_losses = len(loss_weights)
        assert len(loss_functions) == n_losses, "{} loss functions, but {} weights".format(
    ...
    def batch_val(self, batch_data, loss_functions, loss_weights, forward_kwargs=None, custom_kwargs=None):
        loss_contributions = self.forward_pass(
            batch_data, loss_functions, forward_kwargs=forward_kwargs, custom_kwargs=custom_kwargs)
        loss = self.sum_losses(loss_contributions, loss_weights)
...
class SCANDALForwardTrainer(Trainer):
    ...
    def forward_pass(self, batch_data, loss_functions, forward_kwargs=None, custom_kwargs=None):
        ...
        if self.multi_gpu:
            x_reco, log_prob, _ = nn.parallel.data_parallel(
                self.model, x, module_kwargs={"context": params})
        else:
            x_reco, log_prob, _ = self.model(
                x, context=params, **forward_kwargs)

        (t,) = grad(log_prob, params, grad_outputs=torch.ones_like(
            log_prob.data), only_inputs=True, create_graph=True)

        self._check_for_nans("Reconstructed data", x_reco)
        if log_prob is not None:
            self._check_for_nans("Log likelihood", log_prob, fix_until=5)

        losses = [loss_fn(x_reco, x, log_prob, t, t_xz)
                  for loss_fn in loss_functions]
        self._check_for_nans("Loss", *losses)
