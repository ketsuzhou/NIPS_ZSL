...
class NN(nn.Module):
    ...
    Args:
        ...
        num_ConvAttnBlock (int): Number of blocks in the network.
    ...
    """
    def __init__(self, in_channels, mid_channels, num_ConvAttnBlock, num_components, drop_prob, use_attn=True, aux_channels=None):

        super(NN, self).__init__()
        self.k = num_components  # k = number of mixture components
        self.in_conv = WNConv2d(in_channels, mid_channels, kernel_size=3, padding=1)
        self.mid_convs = nn.ModuleList([ConvAttnBlock(mid_channels, drop_prob, use_attn, aux_channels) for _ in range(num_ConvAttnBlock)])
        self.out_conv = WNConv2d(mid_channels, in_channels * (2 + 3 * self.k),
