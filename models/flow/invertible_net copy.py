...
class INVERTIBLE_NET(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_attn=True,
                 drop_prob=0.2):
        super(INVERTIBLE_NET, self).__init__()
    ...
    def forward(self, x, sldj, reverse=False):
        if reverse:
            if self.next is not None:
                # TODO: if the final scale, replaceing the squeeze with DCT transform
                x = squeeze(x)
