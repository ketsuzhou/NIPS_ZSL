...
class MultiscaleCompositeTransform(Transform):
    ...
    def __init__(self, num_transforms, split_dim=1):
        ...
        super().__init__()
        self._modul_list_transforms = nn.ModuleList()
        self._output_shapes = []
        self._num_transforms = num_transforms
        # split_dim begins from one instead of zero
    ...
    def add_transform(self, transform, transform_output_shape):
        ...
        """
        # self._modul_list_transforms: the current constructed list of transforms
        assert len(self._modul_list_transforms) <= self._num_transforms

        if len(self._modul_list_transforms) == self._num_transforms:
            #? more than
        ...
        self._modul_list_transforms.append(transform)
        
        if len(self._modul_list_transforms) != self._num_transforms:  
            # splitting after intermedia transform.
    ...
    def forward(self, inputs, context=None, full_jacobian=False):
        ...
        if self._num_transforms != len(self._modul_list_transforms):
        ...
        def cascade():
            ...
            for i, transform in enumerate(self._modul_list_transforms[:-1]):
            ...
            outputs, logabsdet = self._modul_list_transforms[-1](hiddens, context, full_jacobian)
            yield outputs, logabsdet
    ...
    def inverse(self, inputs, context=None, full_jacobian=False):
        ...
        rev_inv_transforms = [transform.inverse for transform in self._modul_list_transforms[::-1]]
