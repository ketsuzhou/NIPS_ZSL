...
class EncoderManifoldFlow(BaseFlow):
    ...
    def _decode(self, u, mode, u_orthogonal=None, context=None):
        ...
        if u_orthogonal is not None:
            h_manifold_reco = self.projection.inverse(h, orthogonal_inputs=u_orthogonal)
        else:
            h_manifold_reco = self.projection.inverse(h)


        if mode in ["pie", "slice", "projection", "mf-fixed-manifold"]:
            x_reco, inv_log_det_outer = self.outer_transform.inverse(h_manifold_reco, full_jacobian=False, context=context if self.apply_context_to_outer else None)
            inv_jacobian_outer = None
        else:
            x_reco, inv_jacobian_outer = self.outer_transform.inverse(h_manifold_reco, full_jacobian=True, context=context if self.apply_context_to_outer else None)
            inv_log_det_outer = None

        return x_reco, inv_log_det_inner, inv_log_det_outer, inv_jacobian_outer, h_manifold_reco
