...
import numpy as np
# import sys
# sys.path.append('../../manifold_flow')
from manifold_flow import nn as nn_, transforms
# import nn as nn_, transforms
...
def _create_image_transform_step(
    ...
    # performs an affine transformation of the activations using a scale and bias parameter per channel, similar to batch normalization. After initialization, the scale and bias are treated as regular trainable parameters that are independent of the data.
    if actnorm:
        step_transforms.append(transforms.ActNorm(num_channels))

    # channel permutation using OneByOneConvolution
...
def create_image_transform(
    ...
    if multi_scale:
        ...
        for level, level_hidden_channels in zip(range(levels), hidden_channels):
            ...
            #squeeze_transform trades spatial dimensions for channel dimensions
            ...
            transform_level = transforms.CompositeTransform(
                [squeeze_transform] + 
                [_create_image_transform_step(c,
